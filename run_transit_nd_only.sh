...
# Usage: ./run_transit_nd.sh -i input_filelist.txt -r sgRNA_info.txt -m sample_metadata.txt -o output_prefix
...
if [ -z "$input_filelist" ]; then
  echo "Input filelist is required. Usage: ./run_transit_nd.sh -i input_filelist.txt -r sgRNA_info.txt -m sample_metadata.txt -o output_prefix"
  exit 1
fi

# Check if sgRNA info file is provided
if [ -z "$sgRNA_info" ]; then
  echo "sgRNA info file is required. Usage: ./run_transit_nd.sh -i input_filelist.txt -r sgRNA_info.txt -m sample_metadata.txt -o output_prefix"
  exit 1
fi

# Check if sample metadata is provided
if [ -z "$sample_metadata" ]; then
  echo "Sample metadata is required. Usage: ./run_transit_nd.sh -i input_filelist.txt -r sgRNA_info.txt -m sample_metadata.txt -o output_prefix"
  exit 1
fi

# Check if output prefix is provided
if [ -z "$output_prefix" ]; then
  echo "Output prefix is required. Usage: ./run_transit_nd.sh -i input_filelist.txt -r sgRNA_info.txt -m sample_metadata.txt -o output_prefix"
  exit 1
fi
