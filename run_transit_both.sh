...
# Check if base name is provided
if [ -z "$base_name" ]; then
  echo "Base name is required. Usage: ./run_transit.sh -i input_filelist.txt -r sgRNA_info.txt -m sample_metadata.txt -o output_prefix -b base_name -f"
  exit 1
fi
